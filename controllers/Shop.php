    ...
    public function cart()
    {
        ...
        if (is_login()) {
            ...
            curl_setopt_array($curl, array(
              ...
              CURLOPT_POSTFIELDS => 'origin=153&destination='.$this->customer->data()->cities.'&weight=100&courier=jne',
              CURLOPT_HTTPHEADER => array(
                'content-type: application/x-www-form-urlencoded',
                'key: 970107d7f655b04b72855ade5aa63e39'
              ),
            ));
            ...
            $data = $to_obj->rajaongkir->results['0']->costs;
            ...
            $ongkir = ($cart['total_cart'] >= get_settings('min_shop_to_free_shipping_cost')) ? 0 : $min_value ;
        } else {
        ...
        // echo json_encode($this->customer->data()->cities);
    ...
    public function checkout($action = '')
    {
        ...
        switch ($action)
        {
            default :
                ...
                curl_close($curl);
                $to_obj = json_decode($response);
                $kurir = $to_obj->rajaongkir->results['0']->costs;
                $params['kurir'] = $kurir;

                $min_value = PHP_INT_MAX;
                $cheapest_service = [];
                foreach ($kurir as $service) {
                    foreach ($service->cost as $cost) {
                        if ($cost->value < $min_value) {
                            $min_value = $cost->value;
                            $cheapest_service = $service;
                ...
                $params['alamatnya'] = $this->db->query("SELECT CONCAT(b.city_name, ', ' ,b.type,' ' , b.province  ) AS alamatnya FROM `customers` a LEFT JOIN cities b ON a.`cities`=b.`city_id` WHERE a.`user_id`='".get_current_user_id()."' ")->row();

                 
                $subtotal = $this->cart->total();
                // $ongkir = (int) ($subtotal >= get_settings('min_shop_to_free_shipping_cost')) ? 0 : get_settings('shipping_cost');
                $ongkir = (int) ($subtotal >= get_settings('min_shop_to_free_shipping_cost')) ? 0 : $min_value ;
    ...
    function getCost(){
        $curl = curl_init();
            ...
            curl_setopt_array($curl, array(
              ...
              CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
              CURLOPT_CUSTOMREQUEST => 'POST',
              CURLOPT_POSTFIELDS => 'origin=153&destination='.$_GET['q'].'&weight=100&courier=jne',
              CURLOPT_HTTPHEADER => array(
                'content-type: application/x-www-form-urlencoded',
                'key: 970107d7f655b04b72855ade5aa63e39'
              ),
